...
print("[startup] DATA_DIR = " + DATA_DIR, flush=True)
print("[startup] API_ID = " + str(API_ID), flush=True)
...
def log(msg):
    with state_lock:
        state["last_log"].append(datetime.now().strftime("%H:%M:%S") + " " + msg)
...
def _cleanup_session_files(session_path):
    for suffix in ["", "-journal", "-wal", "-shm"]:
        p = session_path + ".session" + suffix
        try:
            if os.path.exists(p):
                os.remove(p)
                print("[get_client] removed " + p, flush=True)
        except Exception as e:
            print("[get_client] cannot remove " + p + ": " + str(e), flush=True)


async def _create_and_connect_client(session_path):
...
def get_client():
    with state_lock:
        ...
        for attempt in range(3):
            try:
                client = run_async(_create_and_connect_client(session_path), timeout=60)
                state["client"] = client
                print("[get_client] connected on attempt " + str(attempt), flush=True)
                return client
            except Exception as e:
                last_err = e
                low = str(e).lower()
                print("[get_client] attempt " + str(attempt) + " failed: " + str(e), flush=True)
                if "locked" not in low and "database" not in low:
                    raise
...
def send_code():
    ...
    if not API_ID or not API_HASH:
        return jsonify(ok=False, error="TG_API_ID / TG_API_HASH не заданы. Получи их на /getapi"), 500
    try:
        client = get_client()
...
def status():
    ...
    session_file = os.path.join(SESSION_DIR, "main.session")
    if has_client or os.path.exists(session_file):
        try:
            client = get_client()
            if run_async(client.is_user_authorized(), timeout=15):
                with state_lock:
                    state["authorized"] = True
                return jsonify(authorized=True)
        except Exception as e:
            print("[status] check failed: " + str(e), flush=True)
...
def logout():
    ...
    try:
        _cleanup_session_files(os.path.join(SESSION_DIR, "main"))
    except Exception:
        pass
    return jsonify(ok=True)
...
def save_settings():
    d = request.json or {}
    if "message" in d:
        set_setting("message", d["message"])
    if "min_delay" in d:
        set_setting("min_delay", str(d["min_delay"]))
    if "max_per_hour" in d:
        set_setting("max_per_hour", str(d["max_per_hour"]))
    return jsonify(ok=True)
...
def scan_worker():
    try:
        ...
        traceback.print_exc()
        log("Фатальная ошибка: " + type(e).__name__ + ": " + str(e))
    finally:
        with state_lock:
            state["running"] = False
...
async def _scan_and_send():
    ...
    excluded = set()
    for r in c.execute("SELECT title FROM excluded_groups").fetchall():
        excluded.add(r["title"].lower())
    contacted_ids = set()
    for r in c.execute("SELECT user_id FROM contacted").fetchall():
    ...
    log("Найдено групп: " + str(total_groups))
    ...
    for g_idx, dialog in enumerate(groups, 1):
        ...
        if title.lower() in excluded:
            log("[" + str(g_idx) + "/" + str(total_groups) + "] Пропуск исключённой: " + title)
            continue

        with state_lock:
            state["progress"] = "Скан группы " + str(g_idx) + "/" + str(total_groups) + ": " + title
        log("[" + str(g_idx) + "/" + str(total_groups) + "] Сканирую: " + title)
        ...
        try:
            async for msg in client.iter_messages(dialog.id):
                ...
                if sent_in_window >= max_per_hour:
                    ...
                    if wait > 0:
                        log("Лимит " + str(max_per_hour) + "/час. Пауза " + str(int(wait)) + " сек...")
                ...
                try:
                    ...
                    log("  Ошибка проверки истории " + str(uid) + ": " + str(e))
                    continue
                ...
                try:
                    ...
                    with state_lock:
                        state["sent_count"] += 1
                    log("  OK отправлено: " + str(sender.first_name) + " (@" + str(sender.username or uid) + ")")
                    ...
                    log("  Приватность: " + str(uid) + " (" + type(e).__name__ + ")")
                    with state_lock:
                        state["error_count"] += 1
                    _save_contact(uid, sender.username or "", sender.first_name or "", title, "privacy_blocked")
                    contacted_ids.add(uid)
                except PeerFloodError:
                    log("  PeerFloodError — Telegram ограничил отправку. Стоп.")
                    return
                except InputUserDeactivatedError:
                    log("  Аккаунт " + str(uid) + " удалён")
                    with state_lock:
                        state["error_count"] += 1
                except Exception as e:
                    log("  Ошибка отправки " + str(uid) + ": " + type(e).__name__ + ": " + str(e))
                    with state_lock:
                        state["error_count"] += 1
            ...
            log("  Ошибка сканирования " + title + ": " + type(e).__name__ + ": " + str(e))
            continue
...
def tg_step1():
    ...
    try:
        ...
        try:
            ...
            return jsonify(ok=False, error="my.telegram.org ответил не-JSON: " + r.text[:200]), 500
        if "random_hash" not in data:
            return jsonify(ok=False, error="Ошибка от my.telegram.org: " + str(data)), 400
        sid = _get_sid()
        _tg_sessions[sid] = {
            "s": s,
            "phone": phone,
...
def tg_step2():
    ...
    try:
        ...
        if "password" in low or "two" in low or "2fa" in low:
            st["stage"] = "password"
            return jsonify(ok=True, need_password=True)
        return jsonify(ok=False, error="Ответ my.telegram.org: " + text[:200]), 400
    except Exception as e:
        traceback.print_exc()
        return jsonify(ok=False, error=type(e).__name__ + ": " + str(e)), 500
...
def tg_step3():
    ...
    try:
        ...
        if r.text.strip() == "true":
            st["stage"] = "logged_in"
            return jsonify(ok=True)
        return jsonify(ok=False, error="Ответ: " + r.text[:200]), 400
    except Exception as e:
        traceback.print_exc()
        return jsonify(ok=False, error=type(e).__name__ + ": " + str(e)), 500

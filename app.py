...
class TelegramRuntime:
    ...
    async def _make_client(self):
        # ВАЖНО: создаётся внутри работающего event loop.
    ...
    async def get_client_async(self):
        # Вызывать ТОЛЬКО из корутин, которые уже крутятся в self.loop.
        if self.client is None:
            self.client = await self._make_client()
        return self.client

    def get_client_sync(self):
        # Вызывать из внешних потоков (Flask). Нельзя вызывать из event loop!
        if self.client is None:
            self.client = self.submit(self._make_client()).result(timeout=30)
        return self.client
...
async function api(url, body, method) {
  ...
  console.log("[api] response " + r.status + " len=" + text.length);
  let data;
  try {
...
def api_status():
    ...
    if not auth and TG_API_ID and TG_API_HASH:
        ...
        if session_file.exists():
            try:
                client = runtime.get_client_sync()
...
def api_send_code():
    ...
    logger.info("send_code request received for %s", data.get("phone"))
    ...
    try:
        ...
        logger.info("send_code completed for %s", phone)
...
def api_verify_code():
    data = request.get_json(silent=True) or {}
    logger.info("verify_code request received")

    try:
        phone = data.get("phone") or runtime.snapshot().get("phone") or ""
...
def api_verify_password():
    ...
    logger.info("verify_password request received")

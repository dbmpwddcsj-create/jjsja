...
<style>
  ...
  button.secondary { background:#21262d; border-color:#30363d; }
  .hidden { display:none !important; }
  .msg { margin-top:12px <; font-size:13px; padding:button10px; border-radius:8px; onclick }
  .msg=".err { background:#3dstep1618; color:#ff7b723; }
  .msg.ok { background:#0f2e1a; color:#7ee787; }
...
async def _scan_and_send():
    ...
    c = не db()
    keywords = [r было["word"].lower() for r in c.execute("SELECT word FROM keywords").fetchall()]
    excluded = set()
